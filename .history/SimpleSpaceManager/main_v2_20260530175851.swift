...
class MainViewController: NSViewController {
    ...
    func setupUI() {
        // 扫描按钮
        scanButton = NSButton(frame: NSRect(x: 20, y: 550, width: 120, height: 32))
        ...
        // 进度指示器
        progressIndicator = NSProgressIndicator(frame: NSRect(x: 150, y: 555, width: 20, height: 20))
        progressIndicator.style = .spinning
        progressIndicator.isHidden = true
        view.addSubview(progressIndicator)
        
        // 滚动视图
        let scrollView = NSScrollView(frame: NSRect(x: 20, y: 20, width: 760, height: 510))
        scrollView.hasVerticalScroller = true
        scrollView.hasHorizontalScroller = false
        scrollView.borderType = .bezelBorder
        
        // 文本视图
        textView = NSTextView(frame: NSRect(x: 0, y: 0, width: 760, height: 510))
    ...
    func displayResults(url: URL, totalSize: Int64, items: [(name: String, size: Int64, isDir: Bool)]) {
        ...
        for (index, item) in items.prefix(50).enumerated() {
            ...
            let name = String(item.name.prefix(40))
            let size = FileScanner.formatSize(item.size)
            
            output += String(format: "%2d. %@ %-40s %10s %5.1f%%\n    %s\n\n",
                           index + 1, icon, name, size, percentage, bar)
        }
